...
def process_data_to_adata(filepath, dataset):
    ...
    res_dec = pd.read_csv("./data/" + dataset + "/deconProp_best.tsv", sep="\t")
...
def eva(y_true, y_pred, epoch=0):
    nmi = metrics.normalized_mutual_info_score(
        y_true.data.cpu(), y_pred, average_method="arithmetic"
    )
    ari = metrics.adjusted_rand_score(y_true.data.cpu(), y_pred)
